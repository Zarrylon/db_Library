...
class Model_db:
    def __init__(self, username, password, host, port, dbname):
        self.__context = psycopg2.connect(user=username, password=password,
    ...
    def delete_with_fk(self, table_name1, table_name2, value1, value2, value3, cond):
        self.__cursor.execute(
            sql.SQL('DELETE FROM {} WHERE {} in (SELECT {} FROM {} WHERE {} = {})').
            format(sql.Identifier(table_name1),
    ...
    # reader subscription
    def join_reader_subscription(self, condition=""):
        return self.join_general("""SELECT  * FROM reader as r
                                JOIN subscription as s
                                ON r.reader_id = s.reader_id {}
                                ORDER BY r.reader_id ASC""", condition)

    # author books
    def join_author_books(self, condition=""):
        return self.join_general("""SELECT  * FROM author as a
                                JOIN books as b
                                ON a.author_id = b.author_id {}
                                ORDER BY b.books_id DESC""", condition)

    # subscription books
    def join_subscription_books(self, condition=""):
        return self.join_general("""SELECT  * FROM subscription as s
                                JOIN books as b
                                ON s.books_id = b.books_id {}
                                ORDER BY b.books_id ASC""", condition)
